  ...
  <div class="max-w-4xl mx-auto p-4 relative overflow-hidden">
    ...
    <div class="grid gap-6 overflow-hidden">
      <div v-for="(item, idx) in items" :key="idx"
        class="border-t flex flex-wrap gap-y-4 pt-6 max-w-full overflow-hidden">
        ...
        <div class="basis-full sm:basis-1/2 order-1 sm:order-2 min-w-0" v-if="item[config.COLUMNS.DRIVE_FOLDER_LINK]">
          ...
          <ImageSwiper v-else-if="itemImages[idx] && itemImages[idx].length > 0" class="w-full min-w-0"
            :key="`swiper-${idx}-${itemImages[idx].length}`" :images="itemImages[idx]" :loading-images="loadingImages"
            @open-lightbox="handleSwiperLightbox" />
    ...
    <div class="fixed pointer-events-none top-3 end-1 z-10 lg:right-[calc(50%-28rem)]">
      ...
      <a class="bg-[#0088cc] block pointer-events-auto rounded-full *:fill-white p-[6px] hover:bg-[#3daae0] border-2 border-white"
